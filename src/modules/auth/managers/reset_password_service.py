...
class ResetPasswordService:
    ...
    def verify(self, email: str, code: str):
        is_verified = self.code_service.validate_code(email, code)
    ...
    def finish(self, email: str, new_password: str):
        has_active_code = self.code_service.has_active_code(email)
        ...
        self.user_service.update_user(email, {"password": password_hash})

        return map_to_message_response("Password updated successfully")

    def get_user(self, token: str):
        email_hash = self.jwt_service.decode(token)
        email = self.encryption_service.decrypt(email_hash)
