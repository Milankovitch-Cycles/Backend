...
class WellService:
    ...
    def create_well(self, name: str, description: str, filename: str, user: UserEntity) -> WellEntity:
        well = WellEntity(name=name, description=description, filename=filename, user_id=user.id, well_metadata={})
        session.add(well)
        session.commit()
    ...
    def create_job(self, well_id: int, type: str, parameters: dict, user: UserEntity):
        job = JobEntity(well_id=well_id, type=type, parameters=parameters, status="in_progress", user_id=user.id, result={})
        session.add(job)
        session.commit()
        session.refresh(job)
        return job
